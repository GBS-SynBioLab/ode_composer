...
# define a MAK model
states = {"x1": "m11*x1+m12*x1*x2", "x2": "m21*x1+m22*x2"}
parameters = {"m11": -0.5, "m12": 0.1, "m21": 0.6, "m22": -0.3}
ss = StateSpaceModel(states=states, parameters=parameters)
print(ss)

# evalute the RHS of the ODE
ss.get_rhs(t=0, y=[0, 1], states=["x1", "x2"], parameters=None)

states = ["x1", "x2"]
parameters = None
t = np.linspace(0, 10, 101)
y0 = [1.2, 1]
sol = solve_ivp(ss.get_rhs, [0, 100], y0, args=(states, parameters))
print(sol)
...
rr = list()
for y in zip(*d):
    rr.append(ss.get_rhs(0, y, states, parameters))
